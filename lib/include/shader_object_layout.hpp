...
vk::ShaderStageFlagBits map_shader_stage(SlangStage stage) {
	switch (stage) {
	default:
...
struct entry_point_shader_layout {
	...
	std::unordered_map<uint32_t, descriptor_set_info>	descriptor_set_infos;
...
public:
	...
	descriptor_set_info& find_or_add_descriptor_set(
...
struct root_shader_layout_builder {
	...
	void add_global_params(slang::VariableLayoutReflection* globals_layout) {
		...
		global_layout_ = global_shader_layout{
			...
			.descriptor_set_infos = builder.descriptor_set_infos_,
		};
	}

	void add_entry_point(slang::EntryPointLayout* entry_point_layout) {
		entry_point_layout_builder builder(entry_point_layout);
		entry_points_.emplace_back(builder.build());
	}
